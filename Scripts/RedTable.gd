...
func add_item_to_table(item: Node):
	if pickup_items.size() < max_items:
		...
		item.visible = true  
		item.scale = Vector2(1.2, 1.2)  # Scale the item to 1.2 to make them smaller and maintain 1:1 ratio
		item.rotation_degrees = -90  # Rotate the item by -90 degrees to match the desired orientation
		item.position = Vector2.ZERO  # Set the item position at (0,0) inside the grid
